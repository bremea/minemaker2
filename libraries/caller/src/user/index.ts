...
import type { ApiPlayer, ApiProfile, ApiUser } from '@minemaker/types';

export async function getMe(client: RestClient): Promise<ApiUser> {
	return await client.request<ApiUser>('GET', 'user/me');
}

export async function getProfile(client: RestClient, username: string): Promise<ApiProfile> {
	return await client.request<ApiProfile>('GET', `user/profile/${username}`);
}
